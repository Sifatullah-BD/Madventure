...
const ThreadDetail = ({ threadId, onBack }) => {
    ...
    if (!thread) return <div className="p-12 text-center text-gray-500">থ্রেড পাওয়া যায়নি!</div>;

    const initials = thread.profiles?.full_name ? thread.profiles.full_name.charAt(0) : '?';

    return (
        <div className="max-w-4xl mx-auto py-8 px-4 sm:px-6">
            <button onClick={onBack} className="flex items-center gap-2 text-gray-500 hover:text-gray-800 font-bold mb-6 transition-colors">
                ...
                <div className="p-6 md:p-8">
                    <div className="flex items-center justify-between mb-6">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h3 className="font-bold text-gray-900">{thread.profiles?.full_name || 'Anonymous'}</h3>
                                <p className="text-xs text-gray-500">{new Date(thread.created_at).toLocaleDateString()} • {new Date(thread.created_at).toLocaleTimeString()}</p>
                            </div>
                        </div>
                        <button className="text-gray-400 hover:text-gray-700 p-2"><MoreVertical size={20}/></button>
                    </div>

                    <h1 className="text-2xl font-bold text-gray-900 mb-4">{thread.title}</h1>
                    <p className="text-gray-700 text-lg leading-relaxed mb-6 whitespace-pre-wrap">{thread.body}</p>

                    {/* Metadata & Tags */}
                    <div className="flex flex-wrap items-center gap-3 mb-6 bg-gray-50 p-4 rounded-xl">
                        {thread.tags && thread.tags.map(tag => (
                            <span key={tag} className="text-sm text-gray-600 bg-white border border-gray-200 px-3 py-1 rounded-full">
                                #{tag}
                            </span>
                        ))}
                    </div>
                    ...
                    <div className="flex justify-between items-center pt-4 border-t border-gray-100">
                        ...
                        <div className="flex gap-3">
                            <button className="text-gray-500 hover:text-[#22c55e] font-bold text-sm flex items-center gap-1.5 transition-colors">
                                <Share2 size={16}/> শেয়ার
                            </button>
                            <button className="text-gray-400 hover:text-red-500 font-bold text-sm flex items-center gap-1.5 transition-colors">
            ...
            <h3 className="font-bold text-xl text-gray-800 mb-6 flex items-center gap-2">
                <MessageSquare className="text-primary"/> উত্তরসমূহ ({thread.replies?.length || 0})
            </h3>
            ...
            <div className="space-y-4 mb-8">
                        ...
                        <div className="flex items-center justify-between mb-4">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-gray-500">{new Date(reply.created_at).toLocaleDateString()} {new Date(reply.created_at).toLocaleTimeString()}</p>
                                </div>
                            </div>
                        </div>
                        
                        <p className="text-gray-700 leading-relaxed md:ml-13">{reply.body}</p>
                        
                        <div className="mt-4 pt-4 border-t border-gray-50 flex items-center justify-between md:ml-13">
                ...
                {(!thread.replies || thread.replies.length === 0) && (
                    <div className="text-center py-8 text-gray-500 italic bg-gray-50 rounded-2xl border border-dashed border-gray-200">
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                <h4 className="font-bold text-gray-800 mb-4">আপনার উত্তর লিখুন</h4>
                <form onSubmit={handleReply}>
                    <textarea 
                        ...
                        className="w-full border border-gray-200 rounded-xl p-4 focus:ring-2 focus:ring-green-50 focus:border-primary outline-none resize-none mb-4"
                    ></textarea>
